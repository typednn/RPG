...
from . import utils
from . import fetch_env, rotations
import numpy as np
import mujoco_py
...
class FetchBlockConstructionEnv(fetch_env.FetchEnv, gym_utils.EzPickle):
    def __init__(self, initial_qpos,
        ...
        path = F"{os.path.dirname(os.path.realpath(__file__))}/assets/fetch/"
        with tempfile.NamedTemporaryFile(mode='wt', dir=path, delete=False, suffix=".xml") as fp:
    ...
    def render(self, mode='human', size=None):
        img = super(FetchBlockConstructionEnv, self).render(mode='rgb_array')
        if mode != 'rgb_array':
            if mode == 'plt':
    ...
    def get_block_pose(self, idx):
        return self.sim.data.get_site_xpos(self.object_names[idx]).copy()


    def get_state(self):
        return np.concatenate([
            self.sim.data.qpos.flat, self.sim.data.qvel.flat, self.goal
        ])

    def set_state(self, state):
        a, b = self.model.nq, self.model.nq + self.model.nv
        qpos = state[:a]
        qvel = state[a:b]
