...
from .end_effector import \
    Gripper, Fingers, Knife, Pusher, \
    PusherWithObstacle, RotatingPusher, ContainerWithObstacle, \
    Tool
...
class MultiToolEnv(GoalEnv):
    ...
    def __init__(
        ...
        render_cfg:CN=None, #Renderer.get_default_config(),
        ...
        self.tools: Dict[str, Tool] = {
            ...
            'Fingers': Fingers(self.simulator),
            'Pusher': Pusher(self.simulator),
            'PusherWithObstacle': PusherWithObstacle(self.simulator),
            'RotatingPusher': RotatingPusher(self.simulator),
            'ContainerWithObstacle': ContainerWithObstacle(self.simulator)
        }
    ...
    def _render_rgb(self, batch_index=0, index=None, **kwargs):
        ...
        if img.shape[-1] >= 3:
            img = np.uint8(img[:, :, :3].clip(0, 1) * 255)
        return img
