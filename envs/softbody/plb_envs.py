...
def plot_pcd(*pcds):
    ...
    for s, m, c in zip(pcds, ['o', '^', '+'], ['r', 'g', 'b']):
        s = tonumpy(s)
        ax.scatter(s[:, 0], s[:, 1], s[:, 2], marker=m, color=c)
...
def subsample_pcd(n, *args):
    label = torch.randint(0, len(args[0]), (n,))
    out = [i[label] for i in args]
    if len(args) == 1: return out[0]
    return out
