...
class MPMSimulator(Configurable):
    def __init__(
        ...
        #device='cuda:' + os.environ.get('CUDA_VISIBLE_DEVICES', '0'),
        device = None
    ):
        super().__init__()
        #self.cfg = cfg
        ...
        if n_bodies:
            self.body_type_mu_softness_K = array(dtype=quat, length=n_bodies)
    ...
    def extend(self, T):
        self.max_steps = max(self.max_steps, T)
        while len(self.states) < T:
            self.states.append(State(self.n_particles, self.n_bodies))
        lib.print_memory_info()
    ...
    def get_state(self, index):
        ...
        return tuple(state[:4]) + tuple([np.r_[a, b] for a, b in zip(state[4][:self.n_bodies], state[5])])
    ...
    def set_softness(self, softness):
        pp = self.body_type_mu_softness_K.download()
        pp[:, 2] = softness
        self.body_type_mu_softness_K.upload(pp)
    ...
    def get_dists(self, f, grad=None, device='cuda:0'):
        ...
        lib.compute_dist(
            ...
            self.body_type_mu_softness_K.data_ptr,
    ...
    def get_tool_state(self, index=0, device=None):
        pos = self.states[index].body_pos.download(self.n_bodies)
        rot = self.states[index].body_rot.download(self.n_bodies)
    ...
    def set_particle_properties(self, mu_lam_yield):
        self.particle_mu_lam_yield.upload(mu_lam_yield)

    def init_bodies(self, types, softness, mu, K, args, action_scales, pos=None, rot=None):
        assert len(mu) == len(args) == self.n_bodies, f"{len(mu)}, {len(args)}, {self.n_bodies}"
        mu = np.float32(mu)
        ...
        self.body_type_mu_softness_K.upload(
            np.stack((types, mu, softness, K), 1).reshape(-1, 4))
    ...
    def get_ground_height(self):
        return float(self._cfg.ground_height)
    ...
    def grid_op(self, state1: State, temp: TempState, state2: State):
        lib.grid_op_v2(
            ...
            self.body_type_mu_softness_K.data_ptr, self.body_args.data_ptr,
    ...
    def set_pose(self, state, pos, rot, stream):
        ...
        #state.body_pos.upload(pos)
        #state.body_rot.upload(rot)
    ...
    def get_torch_scale(self, device=None):
        if self._torch_scale is None:
            self._torch_scale = torch.tensor(np.array(self.action_scales), device=device, dtype=torch.float32)

        if device is not None:
            self._torch_scale = self._torch_scale.to(device)
        return self._torch_scale
