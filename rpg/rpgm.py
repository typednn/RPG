...
from tools.utils import RunningMeanStd, mlp, orthogonal_init, Seq, logger, Identity, ema, CatNet
...
from .utils import compute_value_prefix, masked_temporal_mse
...
class Trainer(Configurable, RLAlgo):
    ...
    def critic_loss(self, obs, init_z, alpha):
        ...
        with torch.no_grad():
            vtarg = self.target_nets.inference(obs, init_z, self.horizon, alpha=alpha)['values']
    ...
    def update_actor(self, obs, init_z, alpha):
        ...
        return {
            ...
            'entropy': float(entropy_term.mean(axis=0)),
        }
