...
trainer = Trainer.parse(env, max_update_step=1000, buffer=dict(priority=False), head=dict(std_mode='statewise', std_scale=1., squash=True), entropy_coef=1., entropy_target=-6, actor_optim=dict(max_grad_norm=1.), update_train_step=0, rew_norm=True) # do not know if we need max_grad_norm
