...
trainer = Trainer.parse(env, update_step=200, buffer=dict(priority=False), head=dict(std_mode='statewise', std_scale=1., squash=True), entropy_coef=1., entropy_target=-6, actor_optim=dict(max_grad_norm=1.), separate_value_dyna=True) # do not know if we need max_grad_norm
trainer.run_rpgm()
