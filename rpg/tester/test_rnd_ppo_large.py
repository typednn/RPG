...
# N = 1
N = 256
env = TorchEnv('LargeMaze', N)

train_ppo.parse(
    env, steps=200, obs_norm=False,
    ...
    batch_size=2000,
    ppo=dict(learning_epoch=2, ignore_done=False, lmbda=0.97, entropy=dict(coef=0.001, target=-1.)),
    ...
    rnd=dict(learning_epoch=20), # no rnd
    path='tmp/rnd/ppo_large',
) # use tanh