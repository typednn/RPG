...
class MazeGame(object):
    ...
    def __init__(self, height=20, width=10):
        self.maze = Maze.generate(height, width)
        self.target = None
    ...
    def _display(self, pos, value, mode):
        """
        Displays a value on the screen from an x and y maze positions.
        """
        x, y = pos
        # Double x position because displayed maze is double-wide.
        return console.set_display(y * 2 + 1, x * 4 + 2, value, mode=mode)

    def reset(self, reset_target=True):
        self.player = self._get_random_position()
        if reset_target or self.target is None:
            self.target = self._get_random_position()

    def render(self, mode='human'):
        console.display(str(self.maze), mode=mode)
        self._display(self.player, '@', mode=mode)
        out = self._display(self.target, '$', mode=mode)
        if out is not None:
            return out[:-1]
...
if __name__ == '__main__':
    ...
    try:
        while MazeGame(width, height).play(): pass
    except:
        import traceback
